...
ARCHITECTURE structural OF riscv_core IS
   ...
   SIGNAL i_instr_data    : std_logic_vector(31 DOWNTO 0);
   ...
   SIGNAL o_regwdsel      : std_logic_vector(2 DOWNTO 0);
   ...
   END COMPONENT;
   COMPONENT pipereg_if_id
   PORT (
      i_clk           : IN     std_logic;
      i_encoded_instr : IN     std_logic_vector (31 DOWNTO 0);
      i_rst           : IN     std_logic;
      o_encoded_instr : IN     std_logic_vector (31 DOWNTO 0)
   );
   END COMPONENT;
   COMPONENT regfile
   PORT (
   ...
   FOR ALL : pipereg_if_id USE ENTITY work.pipereg_if_id;
...
BEGIN
   ...
   BEGIN
      ...
      s_pcplusfour <= std_logic_vector(temp_sum(31 DOWNTO 0));
   ...
   BEGIN
      ...
      temp_carry(0) := '0';
      temp_sum := signed(temp_din0) + signed(temp_din1) + temp_carry;
      s_pcplusimm <= std_logic_vector(temp_sum(31 DOWNTO 0));
   ...
   u_0combo_proc: PROCESS(o_data_addr_internal, i_data_data, s_pcplusimm, 
                          o_instr_addr_internal, s_pcplusfour, 
                          o_regwdsel)
   BEGIN
      CASE o_regwdsel IS
      WHEN "000" => dout5 <= o_data_addr_internal;
      WHEN "001" => dout5 <= i_data_data;
      WHEN "010" => dout5 <= s_pcplusimm;
      WHEN "011" => dout5 <= o_instr_addr_internal;
      WHEN "100" => dout5 <= s_pcplusfour;
   ...
   u_6combo_proc: PROCESS(s_pcplusfour, s_pcplusimm, o_data_addr_internal, 
                          o_pc_ctrl)
   BEGIN
      CASE o_pc_ctrl IS
      WHEN "00" => s_pc_nxt <= s_pcplusfour;
      WHEN "01" => s_pc_nxt <= s_pcplusimm;
   ...
   u_pipereg_if_id : pipereg_if_id
      PORT MAP (
         i_rst           => rst_i,
         i_clk           => clk_i,
         i_encoded_instr => i_encoded_instr,
         o_encoded_instr => i_instr_data
      );
   u_reg_file : regfile
      PORT MAP (
         clk_i       => clk_i,
